...
async function createPayment({ amountEur, chain, webhookUrl = null }) {
    ...
    const address      = chain === 'btc' ? deriveBtc(walletIndex) : deriveEvm(walletIndex);
...
function _startListener(payment) {
    ...
    let listenerPromise;
    if (chain === 'btc') {
        const minSats = Math.floor(parseFloat(amountCrypto) * 1e8 * (1 - PAYMENT_TOLERANCE));
        ...
        const minUnits     = ethers.parseUnits(amountCrypto, 6) * toleranceFactor / 1000n;
        listenerPromise    = waitForUsdc(networkLabel, wsUrl, contractAddr, address, minUnits);
    }
    ...
    listenerPromise
        .then(result => {
            ...
            console.log(`[payments] ${id} confirmed — ${result.amount} ${payment.currency}`);
