...
function printWalletTable(wallets) {
    ...
    console.log(`  ${line('─', 4)} ${line('─', 18)} ${line('─', 44)} ${line('─', 34)}`);
    for (const w of wallets) {
        const lbl = (w.label || '—').padEnd(18);
        console.log(`  ${String(w.index).padEnd(4)} ${lbl} ${deriveEvm(w.index).padEnd(44)} ${deriveBtc(w.index)}`);
    }
    console.log();
}
...
async function actionPay(rl) {
    ...
    if (!wallet) { console.log('  Invalid wallet.\n'); return; }

    console.log('\n  Select network:\n    1) ETH — USDC\n    2) Polygon — USDC\n    3) Bitcoin — BTC');
    const chain = { '1': 'eth', '2': 'polygon', '3': 'btc' }[(await rl.question('\n  Network [1/2/3]: ')).trim()];
    if (!chain) { console.log('  Invalid choice.\n'); return; }

    const currency  = chain === 'btc' ? 'BTC' : 'USDC';
    const amountRaw = (await rl.question(`  Amount (${currency}): `)).trim();
    const amount    = parseFloat(amountRaw);
    if (isNaN(amount) || amount <= 0) { console.log('  Invalid amount.\n'); return; }
    ...
    console.log(`\n${line()}`);
    console.log(`  Wallet  : #${wallet.index}${wallet.label ? ` (${wallet.label})` : ''}`);
    console.log(`  Network : ${networkLabel}`);
    console.log(`  Amount  : ${amountRaw} ${currency}`);
    console.log(`  Address : ${address}`);
    console.log(line());
    console.log(await QRCode.toString(qrContent, { type: 'terminal', small: true }));
    console.log(`  Waiting for ${amountRaw} ${currency}...\n`);

    let result;
    try {
        if (chain === 'eth') {
            result = await waitForUsdc('Ethereum', ETH_WS, USDC_ETH, address, ethers.parseUnits(amountRaw, 6));
        } else if (chain === 'polygon') {
            result = await waitForUsdc('Polygon', POLYGON_WS, USDC_POLYGON, address, ethers.parseUnits(amountRaw, 6));
        } else {
